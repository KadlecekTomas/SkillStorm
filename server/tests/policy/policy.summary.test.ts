...
describe('Policy Compliance Summary', () => {
  afterAll(() => {
    ...
    const completion = score.total ? Math.round((score.passed / score.total) * 100) : 0;
    ...
    for (const category of POLICY_CATEGORIES) {
      ...
      const pct = bucket.total ? Math.round((bucket.passed / bucket.total) * 100) : 0;
    ...
    if (score.failures.length) {
      ...
      for (const f of score.failures) {
        console.log(` - [${f.category}] ${f.description} → ${f.details ?? 'no details'}`);
      }
    } else {
      console.log('\n✅ All policies passed');
    }
