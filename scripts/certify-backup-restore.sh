...
for _ in $(seq 1 60); do
  if curl --silent --show-error --fail --max-time 3 "${BASE_URL:-http://127.0.0.1:3000}" >/dev/null 2>&1; then
    READY=1
    break
  fi
  sleep 2
...
if [[ "$READY" != "1" ]]; then
  log "ERROR: production frontend did not become reachable after restore"
  exit 1
fi
...
LOGIN_STATUS="$(curl --silent --show-error \
  ...
  --data "$LOGIN_BODY" \
  "${BASE_URL:-http://127.0.0.1:3000}/api/auth/login")"

if [[ "$LOGIN_STATUS" != "200" && "$LOGIN_STATUS" != "201" ]]; then
  log "ERROR: restored application login failed status=$LOGIN_STATUS"
  cat "$LOGIN_JSON" | tee -a "$REPORT"
  exit 1
fi
log "restored_login=true"

OVERVIEW_STATUS="$(curl --silent --show-error \
  --output "$OVERVIEW_JSON" \
  --write-out '%{http_code}' \
  --cookie "$COOKIE_JAR" \
  --header "x-org-id: $ORG_ID" \
  "${BASE_URL:-http://127.0.0.1:3000}/api/assignments/overview")"
