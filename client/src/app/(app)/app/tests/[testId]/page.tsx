...
type TestDetail = {
  ...
  subject?: { id: string; name: string; gradeFrom: number; gradeTo: number } | null;
...
function TestPageInner({ testId }: { testId: string }): React.JSX.Element {
  ...
  const fetchTest = useCallback(async (isRefetch = false) => {
    setFetchError(null);
    ...
    try {
      ...
      setTest(data ?? null);
      setFetchError(null);
      if (data?.assignability != null) {
        devLog("refetch success", {
          assignable: data.assignability.isAssignable,
          totalPoints: data.assignability.totalPoints,
          issuesCount: data.assignability.issues?.length ?? 0,
        });
      }
    } catch (e) {
      setTest(null);
  ...
  const handleAddQuestion = useCallback(async () => {
    ...
    const nextOrder = test.questions?.length ?? 0;
    try {
      ...
      setTest((prev) => {
        if (!prev) return prev;
        const nextQuestions = [...(prev.questions ?? []), createdQuestion];
        return { ...prev, questions: nextQuestions };
      });

      if (response?.id) setHighlightQuestionId(response.id);
      ...
      >
        ...
        {test.subject && (
          <p className="mt-1 text-sm text-slate-600">
            Předmět: {test.subject.name} ({test.subject.gradeFrom}–{test.subject.gradeTo})
          </p>
        )}
