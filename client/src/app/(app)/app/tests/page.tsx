...
function subjectColumnRender(row: TestSummary): string {
  ...
  return subjectLabel(sub as OrgSubject);
}
...
function TestsPage(): React.JSX.Element {
  ...
  const { subjects: orgSubjects } = useOrgSubjects();
  ...
  const filteredTests = useMemo(() => {
    ...
    if (subjectFilterId) {
      list = list.filter((t) => (t.subject && typeof t.subject === "object" && (t.subject as OrgSubject).id === subjectFilterId));
    }
    return list;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">
            {isDirector ? "Testy v organizaci" : "Moje testy"}
          </h2>
          <p className="text-sm text-slate-500">
            {isDirector ? "Všechny testy a přiřazení. Filtruj podle učitele." : "Přehled testů, odevzdání a průměry."}
          </p>
        </div>
        <Link
