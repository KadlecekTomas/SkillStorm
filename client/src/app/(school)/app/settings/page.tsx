...
import { PermissionKey, type Subject } from "@/types";
import { useAuth } from "@/hooks/use-auth";
import { subjectLabel } from "@/hooks/use-subjects";
...
export default function SettingsPage(): React.JSX.Element {
  ...
  const loadAllSubjects = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth<Subject[] | { data: Subject[] }>("GET", "/subjects?limit=200&includeInactive=true");
  ...
  const toggleSubjectActive = async (subject: Subject) => {
    ...
    try {
      await fetchWithAuth("PATCH", `/subjects/${subject.id}/activation`, {
        body: { isActive: !subject.isActive },
      });
      setAllSubjects((prev) =>
        prev.map((s) => (s.id === subject.id ? { ...s, isActive: !s.isActive } : s)),
      );
    } catch {
              ...
              {allSubjects.map((subject) => (
                ...
                >
                  ...
                  <Switch
                    ...
                    aria-label={subject.isActive ? `Deaktivovat ${subjectLabel(subject)}` : `Aktivovat ${subjectLabel(subject)}`}
                  />
                </li>
              ))}
            </ul>
