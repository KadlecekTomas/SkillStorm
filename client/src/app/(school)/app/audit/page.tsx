...
const AUDIT_ROLES: OrganizationRole[] = ["OWNER", "DIRECTOR"];
...
function OrgAuditPage(): React.JSX.Element {
  ...
  const load = useCallback(async (f: Filters, p: number) => {
    ...
    try {
      ...
      const data = await httpClient.get<AuditLogsResponse>("/audit/logs", {
        query,
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-slate-500">
        Záznamy jsou automaticky omezeny na vaši organizaci. IP adresa a údaje
        o zařízení nejsou z důvodu ochrany soukromí zobrazovány.
      </p>

      <Card className="border-slate-200 p-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Input
            placeholder="Typ záznamu (USER, TEST…)"
      ...
      <Card className="overflow-hidden border-slate-200">
        <div className="hidden gap-3 border-b border-slate-200 bg-slate-50 px-4 py-2.5 text-xs font-medium uppercase tracking-wide text-slate-500 sm:grid sm:grid-cols-[160px_1fr_1fr_1fr]">
          <span>Čas</span>
          <span>Akce</span>
          <span>Entita</span>
          <span>Uživatel</span>
        ...
        {loading && items.length === 0 ? (
          ...
          <div className="flex flex-col items-center gap-3 py-16">
            ...
            <p className="text-sm text-slate-500">
              Žádné záznamy pro zadané filtry.
            </p>
          </div>
        ) : (
