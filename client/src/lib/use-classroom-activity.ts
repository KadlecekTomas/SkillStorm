...
function eventId(prefix: string, type: LiveSemanticEventType): string {
  const suffix =
    typeof crypto !== 'undefined' && 'randomUUID' in crypto
      ? crypto.randomUUID()
      : `${Date.now()}-${Math.floor(Math.random() * 1_000_000)}`;
  return `${prefix}-${type.toLowerCase().slice(0, 18)}-${suffix}`;
}

export type ClassroomActivityBridge = {
...
export function useClassroomActivity(
  ...
  const emit = useCallback(
    async (
      ...
      const body = {
        eventId: eventId(eventPrefix, type),
    ...
    [eventPrefix, projection, refresh, sessionId],
  );
