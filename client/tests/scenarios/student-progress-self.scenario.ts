...
const unwrap = <T>(body: { data?: T } | T): T =>
  typeof body === 'object' && body !== null && 'data' in body ? (body as { data: T }).data : (body as T);

test.describe('student progress — self-scoped history', () => {
  test('teacher write becomes visible to the same student without any client-supplied student id', async ({ asRole }) => {
    ...
    const writeResponse = await teacherPage.request.post('/api/progress/entries', {
      data: { studentId: student!.id, subjectId: subject!.id, gradeValue: 2, comment, clientMutationId: `student-self-${Date.now()}` },
    });
    ...
    expect(staffRouteAttempt.status(), 'student must not gain staff endpoint even for own id').toBe(403);
    ...
    expect(forbidden.filter((url) => !url.includes(`/api/progress/students/${student!.id}`)), 'legitimate student dashboard must not produce unexpected 403s').toEqual([]);
