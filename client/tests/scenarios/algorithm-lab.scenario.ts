...
test.describe('Interactive IT Lab — Algorithm Lab walkthrough', () => {
  ...
  test('captures the full predict → build → run → diagnose → transfer flow', async ({ page }) => {
    ...
    await expect(page.getByTestId('algorithm-result')).toContainText('Tahle verze ještě nefunguje.', { timeout: 8_000 });
    ...
    await expect(page.getByTestId('execution-trace')).toContainText('1. ↑ Krok → [1, 0]');
    await expect(page.getByTestId('execution-trace')).toContainText('3. ↑ Krok → [1, 1]');
    ...
    await page.getByRole('button', { name: 'Transfer: další mise →' }).click();
    ...
    await expect(page.getByTestId('algorithm-result')).toContainText('Diagnostický bod: krok 1');
    await expect(page.getByTestId('execution-trace')).toContainText('kolize s překážkou');
    await expect(page.getByTestId('algorithm-robot')).toHaveAttribute('data-x', '0');
